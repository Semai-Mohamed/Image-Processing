...
def image_gradient(img):
    """Compute Gx, Gy, gradient magnitude GI and direction thetaI."""
    Gx = get_Gx(img)
    Gy = get_Gy(img)
    GI = np.sqrt(Gx**2 + Gy**2)
    thetaI = np.arctan2(Gy, Gx)
    return Gx, Gy, GI, thetaI


def gradient_orientation_8dirs(img, b=8):
    """Map gradient orientations to b discrete directions using grad_orientation."""
    Gx, Gy, GI, thetaI = image_gradient(img)
    g_o = tools.grad_orientation(Gx, Gy, GI, b=b)
    return Gx, Gy, GI, thetaI, g_o